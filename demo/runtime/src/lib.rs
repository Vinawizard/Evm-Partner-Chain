...
// Frontier EVM imports (minimal)
use pallet_evm::{
	EnsureAddressTruncated, FixedGasWeightMapping, HashedAddressMapping,
};
use fp_rpc::TransactionStatus;
use fp_self_contained::SelfContainedCall;
use ethereum_types::{H160, H256, U256};
use sp_runtime::traits::{DispatchInfoOf, PostDispatchInfoOf, Dispatchable};
...
impl pallet_evm::Config for Runtime {
	...
	type CreateOriginFilter = (); // Allows all creates
	type CreateInnerOriginFilter = (); // Allows all inner creates  
	type GasLimitPovSizeRatio = ConstU64<4>;
	type GasLimitStorageGrowthRatio = ConstU64<160>;
}

pub struct FixedRootStateRoot;
impl sp_core::Get<H256> for FixedRootStateRoot {
	fn get() -> H256 {
		H256::zero()
	}
}
...
impl pallet_ethereum::Config for Runtime {
	type StateRoot = FixedRootStateRoot;
	type PostLogContent = ();
...
// Implementation of SelfContainedCall for RuntimeCall to enable Ethereum transactions
...
impl_runtime_apis! {
	...
	// Thin Ethereum Runtime RPC API (required by MappingSyncWorker)
	impl fp_rpc::EthereumRuntimeRPCApi<Block> for Runtime {
		...
		fn account_basic(address: H160) -> fp_evm::Account {
			pallet_evm::Pallet::<Runtime>::account_basic(&address).0
		}
		...
		) -> (Option<ethereum::BlockV3>, Option<Vec<fp_rpc::TransactionStatus>>) {
			(None, None)
		}
